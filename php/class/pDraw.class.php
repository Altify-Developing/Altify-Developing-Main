     ...
     if ( $Mode == AXIS_FORMAT_METRIC )
      {
       ...
       elseif (abs($Value) >= 10000)
        return(sprintf("%.0f", ($Value/1000))."k".$Unit);
       elseif (abs($Value) >= 1000)
        return(sprintf("%.1f", ($Value/1000))."k".$Unit);
             ...
             if ( $DisplayValues && $Serie["Data"][$Key] != VOID ) 
              {
               ...
               $new_y = round($Y-$Offset-$Weight);

               if (trim($Serie["Data"][$Key]) != '') // do not count offsets for empty (NULL) non-displayable values
               {
	               if (isset($line_offsets[$X]))
	               {
		               	$stack_ok = false;
		               	while (!$stack_ok)
		               	{
		               		$stack_ok = true;
			                for ($i = 0; $i < count($line_offsets[$X]); $i++)
			                {
			                	if (abs($line_offsets[$X][$i]-$new_y) < 6)
			                	{
			                		$new_y -= 6-abs($line_offsets[$X][$i]-$new_y);
			                		$stack_ok = false;
			                		break;
			                	}
			                }
						}
						$line_offsets[$X][] = $new_y;
		               } else
		               {
		               	$line_offsets[$X][] = $new_y;
		               }
               ...
               $this->drawText($X,$new_y,$this->scaleFormat($Serie["Data"][$Key],$Mode,$Format,$Unit),array("R"=>$DisplayR,"G"=>$DisplayG,"B"=>$DisplayB,"Align"=>$Align));
              }
